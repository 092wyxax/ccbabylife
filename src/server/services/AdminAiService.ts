...
/** 後台功能地圖 + 常見操作步驟 — 讓 AI 能教老闆娘怎麼用後台 */
const ADMIN_GUIDE = `
後台功能地圖（左側選單，依分組）：
【總覽】儀表板（今日 KPI、待出貨、低庫存警告、本週節奏任務）
【訂單與客戶】訂單｜LINE 收件匣｜退換貨｜電子發票｜客戶｜會員分級｜評論｜補貨通知｜訂閱
【商品與庫存】商品｜庫存｜進貨單｜採購商｜進貨設定
【行銷與內容】部落格｜行銷活動（LINE 群發・優惠券・Email 群發）｜滿額贈/加購｜電子報｜A/B 測試
【數據分析】報表（營收、客戶分群、留存）｜市場情報
【系統管理】管理員｜稽核紀錄｜系統健康｜AI 設定｜設定（僅店主）

常見操作步驟：
- 出貨流程：訂單 → 點進該筆 → 右側「狀態轉移」依序推進（已付款→日本下單中→商品到日本倉→國際集運中→台灣到港→已出貨）。每次變更系統會自動 LINE 通知客人。出貨時在「物流資訊」填業者與追蹤碼。
- 回覆客人 LINE：LINE 收件匣 → 點對話 → 下方輸入框回覆；可按「AI 草擬」先產生草稿再修改送出。
- 上架新商品：商品 → 新增商品 → 最上方貼日本商品網址（樂天/Amazon JP）→ AI 自動抓品名、價格、寫中文文案與建議售價 → 檢查後儲存。
- 補/調庫存：庫存 → 找到商品 → 調整數量。出貨撿貨用「揀貨單」（可直接列印）。
- 發優惠券：行銷活動 → 優惠券 → 新增（固定金額或 %、低消、使用上限）。
- LINE 群發：行銷活動 → LINE 群發（每週最多 1–2 則，避免被封鎖）。
- 訂單金額調整（僅待付款）：訂單詳情 → 「金額調整」改運費或加收/折讓，需填原因。
- 改匯率/免運門檻：設定（僅店主看得到）→ 營運參數。
- 退換貨處理：退換貨 → 按申請單狀態流轉（待審→通過→收貨→退款）。
- 教 AI 記住店裡規則：AI 設定 → 店家備忘。
- 開新管理員帳號：管理員 →（僅店主）新增，可選角色：店主/經理/客服/採購/編輯。`
